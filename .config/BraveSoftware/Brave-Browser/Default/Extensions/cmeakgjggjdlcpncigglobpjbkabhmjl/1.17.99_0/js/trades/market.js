const marketService = function (ws_url, debug) {
  ...
  HANDLERS['createOffer'] = function (req, sendResponse) {
    ...
    requestGet(communityURL, function (data, err) {
      ...
      chrome.webRequest.onBeforeSendHeaders.addListener(hookerHeaders, {urls: ['https://steamcommunity.com/tradeoffer/*']}, ['requestHeaders', 'blocking', 'extraHeaders']);
      let xhr = new XMLHttpRequest();
  ...
  const tradeAction = function (id, action, sendResponse) {
    requestGet(communityURL, (data, err) => {
      if (err) {
        logger.error('acceptOffer cant give sessionId');
        return sendResponse({
      ...
      let hookerHeaders = (details) => {
        let gotRef = false;
        for (let n in details.requestHeaders) {
          gotRef = gotRef || details.requestHeaders[n].name.toLowerCase() === "referer";
        }
        if (gotRef === false) {
          details.requestHeaders.push({name: "Referer", value: `https://steamcommunity.com/tradeoffer/${id}`});
        }
        return {requestHeaders: details.requestHeaders};
      };
      chrome.webRequest.onBeforeSendHeaders.addListener(hookerHeaders, {urls: ["https://steamcommunity.com/tradeoffer/*"]}, ['requestHeaders', 'blocking', 'extraHeaders']);
      ...
      let xhr = new XMLHttpRequest();
      xhr.open('POST', `https://steamcommunity.com/tradeoffer/${id}/${action}`, true);
      xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded; charset=UTF-8');
      xhr.onload = function () {
        chrome.webRequest.onBeforeSendHeaders.removeListener(hookerHeaders);
        if (xhr.status === 200) {
  ...
  HANDLERS['cancelOffer'] = function (req, sendResponse) {
    if (!RQ.can('cancelOffer', 2000)) {
      return sendResponse({
        success: false,
        error: 429
      });
    }
    tradeAction(req.tradeOfferId, 'cancel', sendResponse);
  };
  HANDLERS['declineOffer'] = function (req, sendResponse) {
    if (!RQ.can('declineOffer', 2000)) {
      return sendResponse({
        success: false,
        error: 429
      });
    }
    tradeAction(req.tradeOfferId, 'decline', sendResponse);
  };
  ...
  WSConnect.on('cancelOffer', function (data, answer) {
    if (!answer) answer = function (res) {
    };
    return HANDLERS['cancelOffer'](data, answer);
  });
  WSConnect.on('declineOffer', function (data, answer) {
    if (!answer) answer = function (res) {
    };
    return HANDLERS['declineOffer'](data, answer);
  });
